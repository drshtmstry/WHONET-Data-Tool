  ...
  watch(srcDir, { recursive: true }, (eventType, filename) => {
    if (filename && (filename.endsWith('.html') || filename.endsWith('.js') || filename.endsWith('.css'))) {
      ...
      debounceTimer = setTimeout(() => {
        console.log(`[Auto-Sync] File changed: ${filename}, refreshing browser tabs...`);
        broadcastReload();
      }, 150);
    }
  });
...
function handleRequest(req, res) {
  ...
  if (path === '/api/open-database' && req.method === 'POST') {
    ...
    req.on('end', () => {
      try {
        ...
        if (customPath) {
          if (!existsSync(customPath)) return sendJson(res, { error: 'File does not exist: ' + customPath }, 400);
        ...
        if (!DB_FILES.includes(filename)) return sendJson(res, { error: 'Invalid database' }, 400);
  ...
  if (path === '/api/upload-database' && req.method === 'POST') {
    ...
    req.on('end', () => {
      try {
        ...
        const count = withDb(db => db.prepare('SELECT COUNT(*) as c FROM Isolates').get());
        sendJson(res, { ok: true, filename: safeName, count: count.c, databases: DB_FILES });
      } catch (e) {
        sendJson(res, { error: 'Failed to process SQLite file: ' + e.message }, 500);
      }
    });
    return;
  }
  ...
  if (path === '/api/stats' && req.method === 'GET') {
    ...
    try {
      withDb(db => {
        ...
        `).get();
        const organisms = db.prepare("SELECT DISTINCT ORGANISM FROM Isolates WHERE ORGANISM != '' ORDER BY ORGANISM").all().map(r => r.ORGANISM);
        const wards = db.prepare("SELECT DISTINCT WARD FROM Isolates WHERE WARD != '' ORDER BY WARD").all().map(r => r.WARD);
        sendJson(res, { 
          total, 
  ...
  if (path === '/api/duplicates' && req.method === 'GET') {
    ...
    try {
      ...
      const search = (urlObj.searchParams.get('search') || '').replace(/'/g, "''");
  ...
  if (path === '/api/isolates' && req.method === 'GET') {
    ...
    try {
      ...
      const pageSize = parseInt(urlObj.searchParams.get('pageSize') || '50');
      const search = (urlObj.searchParams.get('search') || '').replace(/'/g, "''");
      const organism = (urlObj.searchParams.get('organism') || '').replace(/'/g, "''");
      const ward = (urlObj.searchParams.get('ward') || '').replace(/'/g, "''");
      const offset = (page - 1) * pageSize;

      const conditions = [];
      if (search) conditions.push(`(UPPER(SPEC_NUM) LIKE UPPER('%${search}%') OR UPPER(PATIENT_ID) LIKE UPPER('%${search}%') OR UPPER(FULL_NAME) LIKE UPPER('%${search}%'))`);
      ...
      withDb(db => {
        ...
        const totalCount = db.prepare(`SELECT COUNT(*) as c FROM Isolates ${where}`).get().c;
  ...
  if (path === '/api/monthly-amr' && req.method === 'GET') {
    ...
    try {
      withDb(db => {
        ...
        // Others: 'xxx', 'xpa', 'xep', 'xsg', 'nor', 'ora' (normal oral flora), 'vag' (normal vaginal flora)
  ...
  if (path.startsWith('/api/isolate/') && req.method === 'GET') {
    ...
    try {
      ...
      withDb(db => {
        const row = db.prepare('SELECT * FROM Isolates WHERE ROW_IDX = ?').get(rowIdx);
  ...
  if (path === '/api/custom-sql' && req.method === 'POST') {
    ...
    req.on('end', () => {
      try {
        ...
        if (!sql || sql.trim().length === 0) return sendJson(res, { error: 'Empty SQL' }, 400);
  ...
  if (path === '/api/update-field' && req.method === 'POST') {
    ...
    req.on('end', () => {
      try {
        ...
        if (!EDITABLE_FIELDS.includes(field)) return sendJson(res, { error: 'Field not editable' }, 400);
        withDb(db => {
          const result = db.prepare(`UPDATE Isolates SET ${field} = ? WHERE ROW_IDX = ?`).run(value, row_idx);
          sendJson(res, { ok: true, changes: result.changes });
        });
      } catch (e) {
        sendJson(res, { error: e.message }, 500);
